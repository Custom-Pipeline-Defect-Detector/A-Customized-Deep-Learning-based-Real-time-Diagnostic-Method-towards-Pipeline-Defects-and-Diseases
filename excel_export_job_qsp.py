...
TEXT_PATH = "runs/detect/exp75/labels/"
# TEXT_PATH = "runs/detect/exp74/labels/"
IMAGE_PATH = "runs/detect/exp75/"
        ...
        with open(file_path) as f:
            while True:
                ...
                if line:
                    ...
                    text_list = list(map(float, line.split(' ', 9)))

                    # # 计算占比 ，直接加入textlist
                    # # 面积占比
                    # # region_size = text_list[3] * text_list[4]
                    # # img_size = text_list[5] * text_list[6]
                    #
                    # # 候选框最长边和图像最长边 比例 /2.5
...
def find_max_proportion_lists(all_content_dict):
    ...
    for k, v in all_content_dict.items():
        ...
        PL_list, BX_list, TJ_list, CK_list = make_class_list(v)
        # 找出其中各自占比最大得
        ...
        if len(max_TJ_list) != 0:
            ...
            attribute_list = export_attribute_formula(defect_type=2, defect_list=max_TJ_list, defect_img=img_src)
            max_TJ_list.append(str(k) + ".img")
            max_TJ_list.extend(attribute_list)
            files_max_TJ_list.append(max_TJ_list)

        if len(max_CK_list) != 0:
            attribute_list = export_attribute_formula(defect_type=3, defect_list=max_CK_list)
            max_CK_list.append(str(k) + ".img")
            max_CK_list.extend(attribute_list)
            files_max_CK_list.append(max_CK_list)

    return files_max_PL_list, files_max_BX_list, files_max_TJ_list, files_max_CK_list
...
def export_attribute_formula(defect_type, defect_list, defect_img=None):
    ...
    if defect_type == 0:
        ...
        # 画面中心点,充当隧道的中心，就是透视法的终点，画面中芯再往下移动一点就是了,这里取得是画面中心的y值的四分之五
        img_center = [defect_list[5] / 2, -defect_list[6] / 2 * (5/4)]
        # 病害中心
        ...
        min_bound = min(defect_left_bound, (img_w-defect_right_bound), abs(defect_top_bound), abs(img_h + defect_buttom_bound))
        # 病害对角线像素长度
        ...
        #  这里改成图像中心到病害右上角的长度，右上角的坐标（defect_center[0]+defect_w/2， defect_center[1]+defect_h/2）,再除以根号二（1.4）
        distance_imgcenter2defectcenter = math.hypot(img_center[0] - defect_center[0] - defect_w/2,
                                                      img_center[1] - defect_center[1] - defect_h/2) / 1.4
        ...
        if k1 <= k <= k2:
            # defect框更靠近左边界
            if min(defect_left_bound, (img_w-defect_right_bound)) == defect_left_bound:
                # 计算中心连线跟x=0的交点
                x = 0
            # defect框更靠近右边界
            else:
        ...
        if defect_w > img_w * 3 / 5 or defect_h > img_h * 3 / 5 or diagonal_center_distance_ratio > 10:
            ...
            if defect_w >= defect_h:
                ...
                # 高度过高的大图片 弧都一般都大于90了，所以直接计算对角线作为弧长是不准确的
                # if (defect_h > img_h * 3 / 5):
                #     defect_diagonal = defect_diagonal * 1.5
        ...
        elif min_bound == (img_w-defect_right_bound):
    ...
    # BX 得提取公式
        ...
        # 改进一下，小故障的候选框 必然 有一边 会特别小，而预测的时候 会导致 候选框相比故障 会大的明显，所以引入了 候选框的较小的一边，参与计算
        deform_ratio = region_diagonal / max(defect_list[5], defect_list[6]) * defect_list[9] * defect_list[9] * (
    ...
    # TJ 得提取公式
        ...
        if circles is not None:
            ...
            for circle in circles[0, :]:
                # 圆心在框里面
                if defect_left_bound<int(circle[0])<defect_right_bound and defect_buttom_bound<-int(circle[1])<defect_top_bound:
...
def make_class_list(mix_class_list):
    ...
    for i in mix_class_list:
        if i[0] == 0:
            ...
            CK_list.append(i)
    return PL_list, BX_list, TJ_list, CK_list
...
def get_max_proportion(list):
    # i[10] 就是占比
    max_proportion = max((i[10] for i in list), default=0)
    max_proportion_list = []
    for i in list:
        if i[10] == max_proportion:
...
def delete_excel():
    ...
    if len(excel_files) != 0:
        for i in tqdm(excel_files, desc="删除旧文件ing"):
...
def write_excel(list):
    ...
    if list[0][0] == 0:
        ...
        for index, i in enumerate(list):
            ...
            #       '0.879861',0.22222, 01.img, ind_arclength, ind_ruptureArea]
            worksheet.write(index + 1, 0, "Pipe")
            # i[11] 是文件名
            worksheet.write(index + 1, 1, i[11])
            # i[12]  ind_arclength
            worksheet.write(index + 1, 2, i[12])
            # i[13]   ind_ruptureArea
            worksheet.write(index + 1, 3, i[13])
    ...
    # BX
        ...
        for index, i in enumerate(list):
            # i[0] 类别，1-4 是四个坐标，5-8是图片尺寸，9置信度，10占比，11文件名, 12往后是故障属性
            # ['0', '942.5', '384', '399', '672', '1280', '720', '1280', '720', '0.879861',0.22222, 01.img, ATTRIBUTEs]
            worksheet.write(index + 1, 0, "Pipe")
            # i[11] 是文件名
            worksheet.write(index + 1, 1, i[11])
            # diameter 直径
            ...
            worksheet.write(index + 1, 3, i[12])
            # deformArea   比率*1000
            worksheet.write(index + 1, 4, i[12] * 1000)
